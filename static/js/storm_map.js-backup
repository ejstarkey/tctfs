// Storm Map - COMPLETE with hover popups
...
function addLegend() {
    ...
    html += `<div style="border-top: 1px solid #e5e7eb; padding-top: 0.75rem; margin-top: 0.75rem;"><div style="font-weight: 600; margin-bottom: 0.5rem; font-size: 0.85rem;">TRACK</div><div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.25rem;"><div style="width: 30px; height: 3px; background: #3b82f6;"></div><span style="font-size: 0.8rem;">Observed Track</span></div><div style="display: flex; align-items: center; gap: 0.5rem;"><div style="width: 30px; height: 3px; background: #3b82f6; opacity: 0.5;"></div><span style="font-size: 0.8rem;">Forecast Track (50% opacity)</span></div></div>`;
...
function getCategoryColor(cat) {
    return BASIN_SCALES[currentBasin].textColors[cat];
}
...
function plotTrack(track) {
    const scale = BASIN_SCALES[currentBasin];
    const coords = track.map(p => [p.longitude, p.latitude]);
    const smooth = catmullRomSpline(coords);
    
    map.addSource('storm-track', { type: 'geojson', data: { type: 'FeatureCollection', features: [{ type: 'Feature', geometry: { type: 'LineString', coordinates: smooth }}]}});
    map.addLayer({ id: 'storm-track-glow', type: 'line', source: 'storm-track', paint: { 'line-color': '#3b82f6', 'line-width': 8, 'line-blur': 4, 'line-opacity': 0.4 }});
    map.addLayer({ id: 'storm-track-line', type: 'line', source: 'storm-track', paint: { 'line-color': '#3b82f6', 'line-width': 3 }});
...
function addCycloneEye(point) {
    const cat = getCategory(point.vmax_kt || 0);
    const text = getCategoryText(point.vmax_kt || 0);
    const color = getCategoryColor(cat);
    const el = document.createElement('div');
    el.style.cssText = 'width: 80px; height: 80px; cursor: pointer;';
    el.innerHTML = `<div style="position: relative; width: 80px; height: 80px;"><img src="/static/img/tc-eye.png" style="width: 80px; height: 80px; animation: rotate 6s linear infinite;"><div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); font-weight: bold; font-size: 18px; color: ${color}; text-shadow: 0 0 3px white, 0 0 3px white;">${text}</div></div>`;
